...
from sagemaker.mlops.feature_store.feature_processor._config_uploader import ConfigUploader
from sagemaker.mlops.feature_store.feature_processor._enums import FeatureProcessorMode
from sagemaker.mlops.feature_store.feature_processor._event_bridge_rule_helper import (
    EventBridgeRuleHelper,
)
from sagemaker.mlops.feature_store.feature_processor._feature_processor_pipeline_events import (
...
from sagemaker.core.network import SUBNETS_KEY, SECURITY_GROUP_IDS_KEY
...
from sagemaker.mlops.feature_store.feature_processor.lineage._feature_processor_lineage import (
...
def to_pipeline(
    ...
    pipeline_session = PipelineSession(
        boto_session=_sagemaker_session.boto_session,
        default_bucket=_sagemaker_session.default_bucket(),
        default_bucket_prefix=_sagemaker_session.default_bucket_prefix,
    )
    logger.info("Created PipelineSession for pipeline %s", pipeline_name)

    model_trainer = _prepare_model_trainer_from_remote_decorator_config(
    ...
    step_name = "-".join([pipeline_name, "feature-processor"])
    training_step_request_dict = dict(
        name=step_name,
        step_args=step_args,
    )
    logger.info("Created TrainingStep '%s' with step_args", step_name)
...
def _prepare_model_trainer_from_remote_decorator_config(
    ...
    Args:
        ...
        pipeline_session (PipelineSession): Pipeline-aware session that causes
            ModelTrainer.train() to return step arguments instead of launching a job.
        role (str): The IAM role ARN for the training job.
    Returns:
        ModelTrainer: A configured ModelTrainer instance.
    """
    ...
    compute = Compute(
        ...
        volume_size_in_gb=remote_decorator_config.volume_size,
        volume_kms_key_id=remote_decorator_config.volume_kms_key,
    )
    logger.info(
        "Compute: instance_type=%s, instance_count=%s, volume_size=%s",
        remote_decorator_config.instance_type,
        remote_decorator_config.instance_count,
        remote_decorator_config.volume_size,
    )

    # Create Networking config if VPC config is present
    networking = None
    if remote_decorator_config.vpc_config:
        networking = Networking(
            subnets=remote_decorator_config.vpc_config[SUBNETS_KEY],
            security_group_ids=remote_decorator_config.vpc_config[SECURITY_GROUP_IDS_KEY],
            enable_inter_container_traffic_encryption=(
                remote_decorator_config.encrypt_inter_container_traffic
            ),
        )
        logger.info(
            "Networking: subnets=%s, security_groups=%s, encrypt=%s",
            remote_decorator_config.vpc_config[SUBNETS_KEY],
            remote_decorator_config.vpc_config[SECURITY_GROUP_IDS_KEY],
            remote_decorator_config.encrypt_inter_container_traffic,
        )

    # Create StoppingCondition if max_runtime_in_seconds is configured
    stopping_condition = None
    if remote_decorator_config.max_runtime_in_seconds:
        stopping_condition = StoppingCondition(
            max_runtime_in_seconds=remote_decorator_config.max_runtime_in_seconds,
        )

    # Create OutputDataConfig
    output_data_config = OutputDataConfig(
        s3_output_path=s3_base_uri,
        kms_key_id=remote_decorator_config.s3_kms_key,
    )

    # Convert tags from List[Tuple[str, str]] to List[Tag]
    tags = None
    if remote_decorator_config.tags:
        tags = [Tag(key=k, value=v) for k, v in remote_decorator_config.tags]
    ...
    logger.info(
        "Created ModelTrainer with image=%s, instance_type=%s, instance_count=%s",
        remote_decorator_config.image_uri,
        remote_decorator_config.instance_type,
        remote_decorator_config.instance_count,
    )
