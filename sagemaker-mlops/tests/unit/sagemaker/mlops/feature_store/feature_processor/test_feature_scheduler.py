...
@patch(
    "sagemaker.mlops.feature_store.feature_processor.feature_scheduler.Pipeline",
...
@patch(
    "sagemaker.mlops.feature_store.feature_processor._config_uploader.ConfigUploader"
    "._prepare_and_upload_spark_dependent_files",
    return_value=("path_a", "path_b", "path_c", "path_d"),
)
@patch(
    "sagemaker.mlops.feature_store.feature_processor._config_uploader.ConfigUploader._prepare_and_upload_workspace",
...
@patch(
    "sagemaker.mlops.feature_store.feature_processor.lineage."
...
def test_to_pipeline(
    ...
    # Configure RuntimeEnvironmentManager mock to return proper string values
    mock_runtime_manager_instance = mock_runtime_manager.return_value
    mock_runtime_manager_instance._current_python_version.return_value = "3.10"
    mock_runtime_manager_instance.snapshot.return_value = "/tmp/snapshot"
    ...
    # Verify ModelTrainer was configured correctly
    model_trainer_call_kwargs = mock_model_trainer.call_args[1]
    assert model_trainer_call_kwargs["training_image"] == "some_image_uri"
    assert model_trainer_call_kwargs["role"] == EXECUTION_ROLE_ARN
    assert model_trainer_call_kwargs["training_input_mode"] == "File"

    # Verify PipelineSession was passed to ModelTrainer
    assert model_trainer_call_kwargs["sagemaker_session"] == mock_pipeline_session.return_value
    ...
    compute_arg = model_trainer_call_kwargs["compute"]
    assert compute_arg.instance_type == "ml.m5.large"
    assert compute_arg.instance_count == 1
    assert compute_arg.volume_size_in_gb == 30

    # No VPC config was provided, so networking should be None
    assert model_trainer_call_kwargs["networking"] is None

    # max_runtime_in_seconds defaults to 86400 in _JobSettings
    stopping_condition_arg = model_trainer_call_kwargs["stopping_condition"]
    assert stopping_condition_arg.max_runtime_in_seconds == 86400

    # Verify OutputDataConfig
    output_data_config_arg = model_trainer_call_kwargs["output_data_config"]
    assert output_data_config_arg.s3_output_path == f"{S3_URI}/pipeline_name"

    # Verify SourceCode has a command string
    source_code_arg = model_trainer_call_kwargs["source_code"]
    assert source_code_arg.command is not None
    assert len(source_code_arg.command) > 0
    assert "--client_python_version 3.10" in source_code_arg.command

    # No tags on _JobSettings, so tags should be None
    assert model_trainer_call_kwargs["tags"] is None

    # Verify train() was called with input_data_config
    mock_model_trainer.return_value.train.assert_called_once()
    train_call_kwargs = mock_model_trainer.return_value.train.call_args[1]
    assert "input_data_config" in train_call_kwargs

    mock_training_step.assert_called_once_with(
        name="-".join(["pipeline_name", "feature-processor"]),
        step_args=mock_model_trainer.return_value.train.return_value,
        retry_policies=[
            StepRetryPolicy(
                exception_types=[
...
@patch(
    "sagemaker.mlops.feature_store.feature_processor.feature_scheduler._validate_pipeline_lineage_resources",
    return_value=None,
)
@patch(
...
@patch(
    "sagemaker.mlops.feature_store.feature_processor.feature_scheduler.EventBridgeRuleHelper",
...
@patch(
    "sagemaker.mlops.feature_store.feature_processor.feature_scheduler.EventBridgeRuleHelper.describe_rule",
    return_value=None,
)
@patch(
    "sagemaker.mlops.feature_store.feature_processor.feature_scheduler.EventBridgeSchedulerHelper.describe_schedule",
    return_value=None,
)
...
@patch(
    "sagemaker.mlops.feature_store.feature_processor._event_bridge_rule_helper.EventBridgeRuleHelper.describe_rule",
    return_value={"EventPattern": "test-pattern"},
)
@patch(
    "sagemaker.mlops.feature_store.feature_processor._event_bridge_rule_helper.EventBridgeRuleHelper.add_tags"
)
@patch(
    "sagemaker.mlops.feature_store.feature_processor.feature_scheduler."
    "_get_tags_from_pipeline_to_propagate_to_lineage_resources",
...
@patch(
    "sagemaker.mlops.feature_store.feature_processor._event_bridge_rule_helper.EventBridgeRuleHelper.put_target"
)
@patch(
    "sagemaker.mlops.feature_store.feature_processor._event_bridge_rule_helper.EventBridgeRuleHelper.put_rule",
...
@patch(
    "sagemaker.mlops.feature_store.feature_processor._event_bridge_rule_helper.EventBridgeRuleHelper.enable_rule"
)
...
@patch(
    "sagemaker.mlops.feature_store.feature_processor._event_bridge_rule_helper.EventBridgeRuleHelper.disable_rule"
)
...
@patch(
    "sagemaker.mlops.feature_store.feature_processor._event_bridge_rule_helper.EventBridgeRuleHelper.remove_targets"
)
@patch(
    "sagemaker.mlops.feature_store.feature_processor._event_bridge_rule_helper.EventBridgeRuleHelper.delete_rule"
)
